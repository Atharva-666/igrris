...
| Capability | Detail |
|---|---|
| 🛡️ Threat Intelligence | Pre-filter layer that blocks known phishing URLs, blacklisted domains, and malicious IPs using live feeds (URLhaus, OpenPhish). |
...
│
├── 📂 backend/
│   └── 📂 threat_intelligence/ # Enterprise Threat Intelligence Pre-filter
│       ├── engine.py          # Rules evaluation engine
│       ├── updater.py         # Atomic feed updater
│       └── cache.py           # In-memory fast cache
│
...
Before an email reaches the machine learning pipeline, it is scanned against known threat feeds (e.g. OpenPhish, URLhaus) and local blacklists. If a critical threat is found, it is blocked immediately.

### Layer 2: Machine Learning Dataset
